...
import com.memorial.common.constant.StringPool;
import com.memorial.common.enums.RandomType;
...
public class StringUtil {
    ...
    public static String format(CharSequence template, Map<?, ?> map) {
        ...
        for (Map.Entry<?, ?> entry : map.entrySet()) {
            template2 = template2.replace("{" + entry.getKey() + "}",   StrFormatter.toStr(entry.getValue()));
        }
        return template2;
    }
    ...
    public static List<String> castList(String strList) {
        if (strList == null) {
            return null;
        }
        String str = strList.replace("[", "").replace("]", "").replace(" ", "");
    ...
    public static String getStrNo() {
        // 88 + 年月日 + 随机数生成
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyyMMdd");

        return simpleDateFormat.format(new Date()) + (int) ((Math.random() * 9 + 1) * 100000);
    }

    public static String getPreciseStrNo() {
        // 88 + 年月日 + 随机数生成
